...
module.exports = {
    ...
    async getAllThoughts(req, res) {
        try {
            ...
            res.status(400).json('Error: ' + "There was an error getting all thoughts");
        }   
    },
    ...
    async getThoughtById(req, res) {
        try {
            ...
            res.status(400).json({message: "There was an error getting one thought"});
        }
    },
// create a new thought and push the created thought's _id to the associated user's thoughts array field
    async createThought(req, res) {
        try {
            ...
            res.status(400).json('Error: ' + "There was an error creating a new thought");
        }
    },
    ...
    async deleteThought(req, res) {
        try {
            const thoughtDelete = await Thought.findOneAndDelete({ _id: req.params.id });
            if (!thoughtDelete) {
